package dk.jyskit.waf.utils.net;
...
public class IpAddressUtils {
	 ...
	 */
	private static final List<Pair<IpAddress, IpAddress>> NON_ROUTABLE_IPS = new ArrayList<Pair<IpAddress, IpAddress>>();
	static {
		NON_ROUTABLE_IPS.add(ImmutablePair.of(new IpAddress("10.0.0.0"), new IpAddress("10.255.255.255")));
		NON_ROUTABLE_IPS.add(ImmutablePair.of(new IpAddress("172.16.0.0"), new IpAddress("172.31.255.255")));
		NON_ROUTABLE_IPS.add(ImmutablePair.of(new IpAddress("192.168.0.0"), new IpAddress("192.168.255.255")));
	}
	...
	public static boolean isNonRoutable(final IpAddress ipAddress) {
		for (final Pair<IpAddress, IpAddress> range: NON_ROUTABLE_IPS) {
			if (ipAddress.isBetween(range.getLeft(), range.getRight())) {
				return true;
			}
		}
		return false;
