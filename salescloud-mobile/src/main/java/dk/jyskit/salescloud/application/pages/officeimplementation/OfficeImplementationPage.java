...
public class OfficeImplementationPage extends ContentPage {
	public OfficeImplementationPage(PageParameters parameters) {
		super(parameters, Lookup.lookup(PageInfoDao.class).findByPageId(CoreSession.get().getBusinessAreaEntityId(), MobilePageIds.MOBILE_OFFICE_IMPLEMENTATION));
	}
	
	@Override
	protected Panel getMainPanel(String wicketId, PageParameters parameters, PageInfo pageInfo) {
		return new EditOfficeImplementationPanel(wicketId);
	}
}
