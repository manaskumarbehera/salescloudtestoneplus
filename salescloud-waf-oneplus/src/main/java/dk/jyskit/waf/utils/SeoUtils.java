...
public class SeoUtils {
	...
	public static String generateSlug(final String name) {
		...
		for (int i = 0; i < name.length(); i++) {
			...
			if (ArrayUtils.contains(DELETE, c)) {
				// do nothing
			} else if (ArrayUtils.contains(REPLACE_HYPHEN, c)) {
	 ...
	 */
	public static Pair<String, String> parseTwoWordSlug(final String slug) throws IllegalArgumentException {
		...
		return ImmutablePair.of(slugs[0], slugs[1]);
	}
}